...
class P2PNetwork(VGroup):
    """A peer-to-peer network visualization with propagation support."""
    ...
    def __init__(self, scale_factor=0.7, **kwargs):
        super().__init__(**kwargs)
        self.nodes = VGroup()
        self.edges = VGroup()

        for pos in self.NODE_POSITIONS:
            node = Circle(radius=0.18, color=BLUE_C, fill_opacity=0.45, stroke_width=2)
...
class MempoolPool(VGroup):
    """A visual mempool container for pending transactions."""

    def __init__(self, width=3.8, height=2.4, color=BLUE_C, **kwargs):
        super().__init__(**kwargs)
        self.container = RoundedRectangle(
            width=width, height=height, corner_radius=0.15,
            color=color, fill_opacity=0.06, stroke_width=2.5,
        )
        self.title = Text("Mempool", font_size=24, color=color)
        self.title.next_to(self.container, UP, buff=0.12)
        self.tx_slots = VGroup()
        self.add(self.container, self.title, self.tx_slots)

    def get_slot_position(self, index: int):
        start = self.container.get_top() + DOWN * 0.55
